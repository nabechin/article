...
class TalkRoomView(LoginRequiredMixin, TemplateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        talk_rooms = UserOwnTalkRoom.objects.all().filter(
            participant=login_user_id).values('talk_room')
        talk_rooms = UserOwnTalkRoom.objects.all().filter(
            talk_room__in=talk_rooms).exclude(participant=login_user_id)
...
class UserOwnTalkRoomView(LoginRequiredMixin, RedirectView):
    ...
    def get_redirect_url(self, *args, **kwargs):
        ...
        talk_rooms = UserOwnTalkRoom.objects.all().filter(
            participant=login_user.id).values('talk_room')
        talk_rooms = UserOwnTalkRoom.objects.all().filter(
            talk_room__in=talk_rooms, participant=kwargs['sender'])
        if not talk_rooms:
            talk_room = TalkRoom.objects.create(last_message='')
            UserOwnTalkRoom.objects.create(
                participant=login_user, talk_room=talk_room)
            sender = get_user_model().objects.get(pk=kwargs['sender'])
            UserOwnTalkRoom.objects.create(
                participant=sender, talk_room=talk_room)
        else:
